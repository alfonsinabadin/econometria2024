...
Para el cálculo de este índice se considera el logaritmo de los valores ya que el PNB tiene una distribución asimétrica. Además, una vez alcanzado determinado nivel de vida se necesitan grandes cambios en el ingreso para mejorarlo aún más, por lo cual se tiene atenuar el efecto del aumento de ingresos en países más desarrollados.**LEER ESTOOOO**

En el _IDH_ se calcula el índice de PNB como

$$Índice_{PNB} = \frac{ln(PNB) - ln(100)}{ln(75.000)-ln(100)}$$
...
Además, se evalúo la posible incorporación en el cálculo de esperanza de vida y de vida saludable a los 60 años para considerar el impacto de la salud en la población mayor en los distintos países. Sin embargo, se terminó considerado más sencillo utilizar únicamente los indicadores al nacer dado que se facilita la interpretación de la dimensión y estas medidas otorgan una visión más amplia de la longevidad y salud de la población a lo largo de toda su vida.
...
$$Dim_{salud} = \frac{esp\;salud-min_{esp\;salud}}{max_{esp\;salud}-min_{esp\;salud}}$$
En la Figura 1 se puede identificar que los resultado del índice de salud para los países en estudio con ambas metodologías no presentan grandes cambios, ambos cálculos parecen obtener prácticamente los mismos resultados.

\pagebreak

```{r fig.cap="Índice de salud obtenido con la metodología original y la opción 1", fig.height=2, echo=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
...
Algunas cuestiones que limitaron esta decisión fue el hecho de que no todos los países tienen esta medida, mientras que la esperanza de vida al nacer es más común y accesible. Sin embargo, se podría imputar aproximando este valor para los países que no los tienen a través de una modificación la esperanza de vida, de todas formas, no se lleva a cabo en el presente trabajo.

2. Tomar ambas variables y promediarlas. Una vez obtenido el promedio se calcula el índice de este eligiendo el cero natural y objetivo aspiracional con el criterio utilizado anteriormente. 

$$prom=\frac{esp\;vida+esp\;salud}{2} \quad Dim_{salud} = \frac{prom-min}{max-min}$$

En la Figura 2 se puede identificar que, nuevamente, el cálculo no parece cambiar notablemente los resultados. Se observa que el mínimo del Índice de salud disminuye con el cálculo propuesto lo que indica que se está "penalizando" aquellos países con baja esperanza de vida saludable. Sin embargo, se explorarán más alternativas para encontrar una superadora.

```{r fig.cap="Índice de salud obtenido con la metodología original y la opción 2", fig.height=2, echo=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
...
$$Dim_{salud}=Índice_{esp\;vida}Índice_{esp\;salud}$$

```{r fig.cap="Índice de salud obtenido con la metodología original y la opción 3", fig.height=2, echo=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
...
4. Otra opción es tomar los índices de esperanza de vida al nacer y esperanza de vida saludable al nacer, ya calculados con sus respectivos mínimos y máximos, y promediarlos.

$$Dim_{salud}=\frac{Índice_{esp\;vida}+Índice_{esp\;salud}}{2}$$

En la Figura 4 es evidente que los resultados no cambian notablemente, pero que al menos hay una disminución del índice al incorporar la esperanza de vida saludable.

\pagebreak

```{r fig.cap="Índice de salud obtenido con la metodología original y la opción 4", fig.height=2, echo=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
...
Al utilizar el _PNB_ ajustado por PPA para medir la dimensión de nivel estándar de vida decente se esta tomando en cuenta solamente el desarrollo económico de un país para explicar qué tan bueno es el nivel de vida de este. La definición de desarrollo humano, mencionada al principio del presente informe, establece que el crecimiento económico no necesariamente conduce a un mayor bienestar **para todos**.
...
ggplot(data, aes(x = poblacion, y = renta)) +
  ...
  geom_label(aes(x = 70, y = 25, label ="Curva de Lorenz"),
             ...
             label.size = 0) +
  geom_label(aes(x = 45, y = 30, label ="Área A"),
...
Para comprender la lógica, se puede observar en la Figura 1 que el 50% de la población alcanza un 25% del ingreso mientras que un 75% de la población llega a más del 50% del ingreso, es decir, el ingreso que logra el 50% de la población lo alcanza el 25% contiguo, lo cual indica desigualdad ya que el ingreso no estaría equitativamente distribuído.
...
1. Se planteo multiplicar $1-\frac{GINI}{100}$  por el PNB de cada país y luego convertir la métrica resultante en un índice. Para ello se considero el cero natural del cálculo del índice de PNB y como objetivo aspiracional se tomó el valor máximo observado de la nueva variable calculada. De esta forma el cálculo es el siguiente:

$$PNB_{Gini} = PNB * \left(1-\frac{GINI}{100}\right)$$

$$Dim_{Ing}=\frac{\ln(PNB_{Gini})-\ln(100)}{\ln(max(PNB_{Gini}))-\ln(100)}$$

\pagebreak

```{r fig.cap="Índice de PNB obtenido con la metodología original y la opción 1", fig.height=2, echo=FALSE, message=FALSE, warning=FALSE}
...
Esta modificación del cálculo, en comparación con el original, devuelve valores menores y cambian sus medidas descriptivas de posición, como la mediana, lo cual es satisfactorio a fines del objetivo propuesto. Esto se puede observar en la Figura 6.
...
$$Dim_{Ing} = Índice_{PNB} * \sqrt{1-\frac{GINI}{100}}$$

En la Figura 7 se puede observar el cambio abrupto mencionado al no utilizar la raíz en la corrección y cómo la segunda opción logra ser un "equilibrio" entre la versión original y la primera modificación realizada con este enfoque.

```{r fig.cap="Índice de PNB obtenido con la metodología original y los diferentes enfoques de la opción 2", fig.height=2, echo=FALSE, message=FALSE, warning=FALSE}
...
Después de analizar ambas opciones, se consideró mejor la multiplicación del coeficiente de Gini previo al cálculo del índice es decir la opción 1, ya que la otra opción penaliza considerablmenete los valores del indice de dimensión de nivel estandar de vida decente.
...
-	Índice de salud: reemplazar la esperanza de vida al nacer por la esperanza de vida saludable al nacer [^1].
$$Dim_{salud} = \frac{esp\;salud-min_{esp\;salud}}{max_{esp\;salud}-min_{esp\;salud}}$$
[^1]: Fuente: [\textcolor{blue}{\underline{World Health Organization, The Global Health Observatory. }}] (https://www.who.int/data/gho/data/themes/topics/indicator-groups/indicator-group-details/GHO/life-expectancy-and-healthy-life-expectancy).

-	Índice económico: multiplicar $1-\frac{GINI}{100}$  por el PNB de cada país y luego convertir la métrica resultante en un índice[^2].
$$PNB_{Gini} = PNB * \left(1-\frac{GINI}{100}\right)$$
$$Dim_{Ing}=\frac{\ln(PNB_{Gini})-\ln(100)}{\ln(max(PNB_{Gini}))-\ln(100)}$$
[^2]: Fuente: [\textcolor{blue}{\underline{Banco Mundial.}}] (https://datos.bancomundial.org/indicator/SI.POV.GINI).

-	Índice de Conocimiento: permanece sin modificaciones.

-	Índice de Libertad: Se toma en consideración a la totalidad de las preguntas realizadas en el índice de libertad “Freedom in the World” del año 2019 (ACLARAR EL AÑO TAMBIEN ARRIBA QUE NO ESTA), publicado por la organización Freedom House[^3].

$$Dim_{Libertad} = Puntaje/100$$
[^3]: Fuente: [\textcolor{blue}{\underline{Freedom House.}}] (https://freedomhouse.org/report/freedom-world).
...
Índice sin Libertad:
$$IDH_{sin \ Libertad} = (Índice_{Salud \ modificado}*Índice_{Educación}*Índice_{Ingresos \ modificado})^{1/3}$$

Índice con Libertad:
$$IDH_{con \ Libertad} = (Índice_{Salud \ modificado}*Índice_{Educación}*Índice_{Ingresos \ modificado}*Índice_{Libertad})^{1/4}$$
...
Se observa una notable estabilidad en los valores del índice de Gini desde el año 2010 hasta el 2019, con algunos cambios más importantes identificados en el año 2020. Además se tienen pocos datos en los años posteriores a la pandemia. Por lo tanto, se decide utilizar el dato de Gini de 2019 o el más reciente disponible de cada país, hasta el año 2010 como máximo, para garantizar que todos los datos sean lo más actualizados posibles, anulando el efecto de la pandemia y la poca cantidad de observaciones de los años más recientes 2022 y 2023.
